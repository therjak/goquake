...
qmodel_t *Mod_FindName(const char *name) {
  ...
  if (!name[0])
    Go_Error("Mod_FindName: NULL name");  // johnfitz -- was "Mod_ForName"
  ...
  if (i == mod_numknown) {
    if (mod_numknown == MAX_MOD_KNOWN)
      Go_Error("mod_numknown == MAX_MOD_KNOWN");
...
void Mod_LoadTextures(lump_t *l) {
  ...
  for (i = 0; i < nummiptex; i++) {
    ...
    for (j = 0; j < MIPLEVELS; j++) mt->offsets[j] = LittleLong(mt->offsets[j]);

    if ((mt->width & 15) || (mt->height & 15))
      Go_Error_S("Texture %v is not 16 aligned", mt->name);
    ...
    if (((byte *)(mt + 1) + pixels) > (mod_base + l->fileofs + l->filelen)) {
      ...
      pixels =
          q_max(0, (mod_base + l->fileofs + l->filelen) - (byte *)(mt + 1));
    }
    ...
    if (!CMLDedicated())  // no texture uploading for dedicated server
    {
      ...
      else if (tx->name[0] == '*')  // warping texture
      {
        ...
        if (data)  // load external image
        {
          ...
          tx->gltexture =
              TexMgrLoadImage2(loadmodel, texturename, fwidth, fheight,
      ...
      } else  // regular texture
      {
        ...
        {
          tx->gltexture =
              TexMgrLoadImage2(loadmodel, filename, fwidth, fheight, SRC_RGBA,
          ...
          if (data)
            tx->fullbright = TexMgrLoadImage2(loadmodel, filename2, fwidth,
                                              fheight, SRC_RGBA, data, filename,
                                              0, TEXPREF_MIPMAP | extraflags);
        } else  // use the texture from the bsp file
        {
          q_snprintf(texturename, sizeof(texturename), "%s:%s", loadmodel->name,
                     tx->name);
          ...
          if (Mod_CheckFullbrights((byte *)(tx + 1), pixels)) {
            ...
            tx->gltexture =
                TexMgrLoadImage2(loadmodel, texturename, tx->width, tx->height,
...
void Mod_LoadLighting(lump_t *l) {
  ...
  // mark = Hunk_LowMark();
  int length = 0;
  data = (byte *)COM_LoadFileGo(litfilename, &length);
  if (data) {
...
void Mod_LoadEntities(lump_t *l) {
  ...
  // mark = Hunk_LowMark();
  int length = 0;
  ents = (char *)COM_LoadFileGo(entfilename, &length);
  if (ents) {
...
void Mod_LoadVertexes(lump_t *l) {
  ...
  if (l->filelen % sizeof(*in))
    Go_Error_S("MOD_LoadBmodel: funny lump size in %v", loadmodel->name);
...
void Mod_LoadNodes_L1(lump_t *l) {
  ...
  if (l->filelen % sizeof(*in))
    Go_Error_S("Mod_LoadNodes: funny lump size in %v", loadmodel->name);
...
void Mod_ProcessLeafs_S(dsleaf_t *in, int filelen) {
  ...
  if (filelen % sizeof(*in))
    Go_Error_S("Mod_ProcessLeafs: funny lump size in %v", loadmodel->name);
...
void Mod_LoadClipnodes(lump_t *l, qboolean bsp2) {
  ...
  if (bsp2) {
    ...
    if (l->filelen % sizeof(*inl))
      Go_Error_S("Mod_LoadClipnodes: funny lump size in %v", loadmodel->name);
...
void Mod_LoadMarksurfaces(lump_t *l, int bsp2) {
  ...
  if (bsp2) {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (j >= loadmodel->numsurfaces)
        Go_Error("Mod_LoadMarksurfaces: bad surface number");
      out[i] = loadmodel->surfaces + j;
    }
  }
}
...
void Mod_LoadBrushModel(qmodel_t *mod, void *buffer) {
  ...
  // we're looping through the submodels starting at 0.  Submodel 0 is the main
  // model, so we don't have to
  // worry about clobbering data the first time through, since it's the same
...
void Mod_SetExtraFlags(qmodel_t *mod) {
  ...
  // nolerp flag
  if (nameInList(Cvar_GetString(&r_nolerp_list), mod->name))
    mod->flags |= MOD_NOLERP;

  // noshadow flag
  if (nameInList(Cvar_GetString(&r_noshadow_list), mod->name))
...
void Mod_LoadAliasModel(qmodel_t *mod, void *buffer) {
  ...
  if (pheader->numverts <= 0) Go_Error_S("model %v has no vertices", mod->name);

  if (pheader->numverts > MAXALIASVERTS)
    Go_Error_S("model %v has too many vertices", mod->name);

  pheader->numtris = LittleLong(pinmodel->numtris);

  if (pheader->numtris <= 0) Go_Error_S("model %v has no triangles", mod->name);
