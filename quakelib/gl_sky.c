...
void Sky_LoadTexture(texture_t *mt) {
  ...
  //      TexMgrLoadImage(loadmodel, texturename, 128, 128, SRC_INDEXED,
  //      back_data,
...
void Sky_DrawFaceQuad(glpoly_t *p) {
  ...
  if (gl_mtexable && Cvar_GetValue(&r_skyalpha) >= 1.0) {
    ...
    if (Cvar_GetValue(&r_skyalpha) < 1.0)
      glColor4f(1, 1, 1, Cvar_GetValue(&r_skyalpha));
...
void Sky_DrawFace(int axis) {
  ...
  di = q_max((int)Cvar_GetValue(&r_sky_quality), 1);
  qi = 1.0 / di;
  dj = (axis < 4)
           ? di * 2
...
void Sky_DrawSky(void) {
  ...
  //
  if (!Cvar_GetValue(&r_fastsky) && !(Fog_GetDensity() > 0 && skyfog >= 1)) {
