...
static void PF_aim(void) {
  ...
  if (tr.ent && tr.ent->v.takedamage == DAMAGE_AIM &&
      (!Cvar_GetValue(&teamplay) || ent->v.team <= 0 ||
...
static void PF_WriteString(void) {
  int dest = G_FLOAT(OFS_PARM0);
  const char *msg = G_STRING(OFS_PARM1);
  if (dest == MSG_ONE) {
...
static void PF_makestatic(void) {
  ...
  if (bits & B_LARGEMODEL)
    SV_SO_WriteShort(SV_ModelIndex(PR_GetString(ent->v.model)));
  else
    SV_SO_WriteByte(SV_ModelIndex(PR_GetString(ent->v.model)));

  if (bits & B_LARGEFRAME)
    SV_SO_WriteShort(ent->v.frame);
  else
    SV_SO_WriteByte(ent->v.frame);
  // johnfitz
...
static void PF_setspawnparms(void) {
  ...
  for (i = 0; i < NUM_SPAWN_PARMS; i++)
    (&pr_global_struct->parm1)[i] = GetClientSpawnParam(client, i);
}
