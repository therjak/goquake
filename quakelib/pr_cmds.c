...
static void SetMinMaxSize(int e, float *minvec, float *maxvec,
                          qboolean rotate) {
  float *angles;
  vec3_t rmin, rmax;
  float bounds[2][3];
  ...
  // set derived values
  VectorCopy(rmin, EVars(e)->mins);
  VectorCopy(rmax, EVars(e)->maxs);
  VectorSubtract(maxvec, minvec, EVars(e)->size);
...
static void PF_setmodel(void) {
  ...
  EVars(e)->model = mi;
  EVars(e)->modelindex = i;  // SV_ModelIndex (m);

  mod = sv.models[(int)EVars(e)->modelindex];  // Mod_ForName (m, true);
